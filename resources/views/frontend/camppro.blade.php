<?php
  if (Session::has('cid')) {  
          //for fetching session value
    foreach($users as $row) {
      $cpm_name = $row->cpm_name;
      $cpm_category = $row->cpm_category;
      $cpm_region = $row->cpm_region;
    }
  }
?>
          ...
          <div class="reset-form camp-form1">
            ...
            <form class="" method="post" action="{{ route('profile_campaign1b') }}" > 
              <div class="form-group">
                <input type="text" class="form-control" name = "cname" id="cname" value="<?php if(isset($cpm_name)) {
                            echo "$cpm_name"; } ?>"          placeholder="Campaign Name" maxlength="105" required autofocus/>
              </div>

              <div class="form-group">
                  <select class="selectpicker form-control" name = "ccateg" id="ccateg" onchange="showfield(this.options[this.selectedIndex].value)">
                  <option selected><?php if(isset($cpm_category)) {echo $cpm_category;} 
              ...
              <div class="form-group">
                  ...
                  <option selected><?php if(isset($cpm_region)) {echo $cpm_region;} 
                                                           else{echo "Select Category";}?></option>
